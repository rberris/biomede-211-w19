...
Active - generates its own energy
	i.e - generators, batteries, op-amps, in general an element that is classified as a source
Passive - cannot generate its own energy
	i.e - resistors, capacitors, conductors, in general an element that processes the energy or electrical charge in 	 some way by storing it or resisting it

Here is a video that explains the difference between active and passive more in depth and provides some additional information on circuit elements and their classification as active or passive.
https://www.youtube.com/watch?reload=9&v=fonmxO0Zf90

\section{Ohm's Law and what it means}

There are two ways to express Ohm's Law. 
	1) V = iR 
		V = potential
		i = current
		R = resistance
	This law describes the potential drop seen on either side of a resistor when some current is flowing through a 		cirucit.
 ** CAUTION ** 
 	V is not equal to a single value. V is really equal to the potential difference or drop across the resistor and so 
	V = V2 - V1. 
	
	2) E = ρJ 
		E = electric field vector
		ρ = resistivity (in short a material's resistance to flow)
		J = current density

Here is a video that gives a more detailed explanation of what a potential difference really is and how it relates to electricity and the flow of current. There are many analogies presented in the video that make differentiating between voltage and current more simple. 
https://www.youtube.com/watch?v=DwMfGEtQUCk
